...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.toSeq.groupBy(charact => charact) map (charTuple => (charTuple._1, charTuple._2.size))
  }.toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    (occurrences foldRight List[Occurrences](Nil)) {
      case ((charLetter,repetitions), acc) => {
        acc ++ ( for { comb <- acc; n <- 1 to repetitions } yield (charLetter, n) :: comb )
      }
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val substractionMap: Map[Char, Int] = y.toMap withDefaultValue 0
    x.map(item => (item._1, item._2 - substractionMap(item._1))).filter(item => item._2 > 0)
  }
